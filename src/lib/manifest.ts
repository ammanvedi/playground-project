...
- **Pending** — the number of tasks with status **"todo"** or **"in-progress"** that have not yet been completed
- **In progress** — the number of tasks currently being worked on (status **"in-progress"**)
- **Completed** — the number of tasks that have been finished (status **"completed"**)
...
Each task is shown as a *task row* — a horizontal strip spanning the full width, separated from other rows by thin **dark gray** borders. When you hover over a row, it subtly highlights with a slightly lighter background and the arrow on the right slides to the right.

Every row contains:

- A small circular *status dot* on the left, color-coded by the task's current state: **dark gray** for todo, **amber** for in-progress, and **green** for completed
- The task name in **white**, medium-weight text that fills the available space. If the name is very long, it is truncated with an ellipsis
- The due date shown in **gray** text on the right (formatted like "Apr 20, 2026")
...
The *text input* has a transparent background with a thin **dark gray** underline border that brightens to **light gray** when focused. Placeholder text reads **"Task name..."** in a dim **dark gray** color.
...
A small uppercase **gray** label reads **"Display name"** above a *text input field*. The input has a transparent background with a thin **dark gray** underline border. When you focus it, the border brightens to **light gray**. Your current name appears in **white** text inside the field. If the field is empty, placeholder text reads **"Your name"** in dim **dark gray**.

Below the input, a small **gray** hint reads "Auto-saved to local storage." — this means **every keystroke saves your change immediately** without needing to press a save button. Your name persists across browser sessions.
...
Below that is a **"Reset data"** *outlined button* — it has a transparent background with a thin **dark gray** border and **white** text. When you click it the first time, **the button changes to a red "Confirm reset" button** as a safety measure. A second *outlined button* labelled **"Cancel"** appears beside it so you can back out. Clicking **"Confirm reset"** erases all your tasks and restores the four default seed tasks. **This action cannot be undone**.
...
Each archived task is displayed as a *task row* identical in layout to the home page — a *status dot*, task name, due date, and arrow — but with one addition: a **"Restore"** *outlined button* sits at the far right of each row. This button has a transparent background, a thin **dark gray** border, and smaller **white** text.
...
When there are no archived tasks, the section displays the centered message **"No archived tasks yet."** in **gray** text.
`,
  },
  {
    url: '/todo/:id',
...
- **Name** — a *text input field* with a thin **dark gray** underline border. It holds the task name in **white** text and saves as you type. The border brightens to **light gray** when focused.
...
- **Status** — a *dropdown select* showing the current status. Next to it sits a small circular *status dot* whose color updates immediately when you change the selection:
  - **"Todo"** — **dark gray** dot
...
- **Due date** — a *date picker input* that shows the current due date. It uses the browser's native date selector and appears with **white** text and a **dark gray** underline border. You can pick any date.

- **Created** — a read-only line of **gray** text showing when the task was first created (e.g. "April 10, 2026 at 20:31"). This field cannot be edited.
...
**Archive** — an *outlined button* with a transparent background, thin **dark gray** border, and **white** text. This button **only appears when the task's status is "completed"**. Clicking it moves the task to archived status and redirects you to the Archive page.

**Delete** — an *outlined button* styled identically to the archive button. When you click it the first time, **the button changes to a red "Confirm delete" button** as a safety step. A **"Cancel"** *outlined button* appears beside it so you can back out. Clicking **"Confirm delete" permanently removes the task** — it cannot be recovered. After deletion, you are redirected to the home page.
